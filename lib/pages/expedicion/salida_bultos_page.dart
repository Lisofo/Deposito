...
class VerificacionBultoVirtualPage extends StatefulWidget {
  const VerificacionBultoVirtualPage({super.key});

  @override
  VerificacionBultoVirtualPageState createState() => VerificacionBultoVirtualPageState();
}

class VerificacionBultoVirtualPageState extends State<VerificacionBultoVirtualPage> {
  ...
  Future<void> _cargarDatosIniciales() async {
    ...
    if (entrega.entregaId != 0) {
      ...
      // Si no existe bulto virtual, crear uno nuevo
      if (_bultoVirtual == null && !_vistaMonitor) {
  ...
  Future<void> procesarEscaneoUbicacion(String value) async {
    if (value.isEmpty || _bultoVirtual == null || _vistaMonitor) return;
  ...
  Future<void> _cerrarBultoVirtual(String comentario, bool incluyeFactura) async {
    try {
      ...
      await EntregaServices().putBultoEntrega(
        ...
        _bultoVirtual!.tipoBultoId, // tipoBultoId
        incluyeFactura,
  ...
  Future<void> _procesarSiguiente() async {
    if (_vistaMonitor) return;
    
    if (!_validarCompletitudProductos()) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Debe verificar todos los productos antes de continuar'),
    ...
    try {
      // Cerrar el bulto virtual
      await _cerrarBultoVirtual('Bulto virtual cerrado automáticamente', false);

      // Verificar si es orden de MOSTRADOR
      if (_ordenSeleccionada?.metodoEnvio == 'MOSTRADOR') {
        // Cerrar la entrega completamente
        ...
        // Navegar a pantalla de éxito o volver atrás
        Navigator.of(context).pop();
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Entrega completada exitosamente')),
        );
      } else {
        // Navegar a la pantalla de creación de bultos
        final productProvider = Provider.of<ProductProvider>(context, listen: false);
        productProvider.setEntrega(entrega);
        productProvider.setOrdenesExpedicion(_ordenes);
        
        appRouter.push('/creacionBultosPage');
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al procesar: ${e.toString()}')),
      );
    } finally {
      if (mounted) {
        setState(() {
                            ...
                            child: ListTile(
                              ...
                              trailing: Row(
                                ...
                                children: [
                                  ...
                                  if (!_vistaMonitor && itemVerificado != null) ...[
                                    IconButton(
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(
            _vistaMonitor ? 'Monitor de Verificación - Entrega: ${entrega.entregaId}' : 'Verificación de Productos - Entrega: ${entrega.entregaId}', 
            ...
            : BottomAppBar(
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      // Text(
                      //   'Productos verificados: ${_validarCompletitudProductos() ? 'COMPLETO' : 'INCOMPLETO'}',
                      //   style: TextStyle(
                      //     fontWeight: FontWeight.bold,
                      //     color: _validarCompletitudProductos() ? Colors.green : Colors.orange,
                      //   ),
                      // ),
                      _procesandoCierre
